      ...
      <table class="mx-5 my-5 w-[95%]">
        ...
        <tbody class="bg-white">
          ...
          >
            ...
            <td class="border-b px-4 py-2 w-[16%]">{{ item.HoTen }}</td>
            <td class="border-b px-4 py-2 w-[16%]">{{ item.DienThoai }}</td>
            <td class="border-b px-4 py-2 w-[16%]">{{ item.DiaChi }}</td>
            <td class="border-b px-4 py-2 w-[16%] mx-auto">
              {{ item.ChucVu }}
            </td>
            <td  class="border-b px-4 py-2 w-[10%]">
              <i v-if="item.ChucVu!='admin'"
                class="fa-solid fa-pencil p-2 ml-3 cursor-pointer"
                style="color: #ffd43b"
                @click="editStaff(item._id)"
              ></i>
              <i 
                v-if="item.XOA == 0 && item.ChucVu!='admin'"
                class="cursor-pointer p-2 fa-solid fa-lock"
                style="color: #b12020"
                @click.prevent="lockStaff(item._id)"
              ></i>
              <i
                v-if="item.XOA == 1 && item.ChucVu!='admin'"
...
const lockStaff = async (id) => {
  try {
    const lock = await axios.patch(`http://localhost:3000/api/lockStaff/${id}`);
    alert("Thành công");
    fetchData();
  } catch (err) {
    alert("Thất bại");
