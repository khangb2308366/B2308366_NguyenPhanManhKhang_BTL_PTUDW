    ...
    <main class="flex-1 p-8 md:p-12 overflow-y-auto h-screen">
      ...
      <div v-if="activeTab === 'dashboard'" class="animate-in fade-in duration-500">
        <header class="mb-12 flex justify-between items-end">
          <div>
            <h1 class="text-4xl font-black tracking-tight text-slate-900">Tổng quan Thư viện</h1>
            <p class="text-slate-500 font-medium mt-1 uppercase text-[10px] tracking-[0.3em]">Số liệu thống kê thời gian thực</p>
          </div>
        </header>

        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          <div class="bg-white p-6 rounded-[2rem] shadow-sm border-l-8 border-blue-500 hover:shadow-md transition-all group">
            <p class="text-slate-400 text-[10px] font-black uppercase mb-1">Đầu sách</p>
            <div class="flex justify-between items-center"><h3 class="text-3xl font-black">{{ stats.totalBooks }}</h3><i class="fa-solid fa-book text-blue-100 text-2xl"></i></div>
          </div>
          <div class="bg-white p-6 rounded-[2rem] shadow-sm border-l-8 border-emerald-500 hover:shadow-md transition-all group">
            <p class="text-slate-400 text-[10px] font-black uppercase mb-1">Trong kho</p>
            <div class="flex justify-between items-center"><h3 class="text-3xl font-black text-slate-800">{{ stats.stockBooks }}</h3><i class="fa-solid fa-warehouse text-emerald-100 text-2xl"></i></div>
          </div>
          <div class="bg-white p-6 rounded-[2rem] shadow-sm border-l-8 border-orange-500 hover:shadow-md transition-all group">
            <p class="text-slate-400 text-[10px] font-black uppercase mb-1">Chờ duyệt</p>
            <div class="flex justify-between items-center"><h3 class="text-3xl font-black text-orange-600">{{ stats.pendingRequests }}</h3><i class="fa-solid fa-clock-rotate-left text-orange-100 text-2xl animate-pulse"></i></div>
          </div>
          <div class="bg-white p-6 rounded-[2rem] shadow-sm border-l-8 border-indigo-500 hover:shadow-md transition-all group">
            <p class="text-slate-400 text-[10px] font-black uppercase mb-1">Nhân sự</p>
            <div class="flex justify-between items-center"><h3 class="text-3xl font-black text-indigo-600">{{ staffList.length }}</h3><i class="fa-solid fa-user-shield text-indigo-100 text-2xl"></i></div>
          </div>
        </div>
        ...
        <div class="mt-12 bg-white p-10 rounded-[3rem] shadow-sm border border-slate-200 flex flex-col md:flex-row items-center justify-between gap-6">
          <div class="max-w-md text-center md:text-left">
            <h2 class="text-2xl font-black mb-2 text-slate-800">Tác vụ nhanh</h2>
            <p class="text-slate-500 text-sm leading-relaxed font-medium">Thêm sách mới vào hệ thống hoặc quản lý đơn mượn đang chờ xử lý.</p>
          </div>
          <button @click="openAddModal" class="bg-slate-900 hover:bg-blue-600 text-white px-8 py-4 rounded-2xl font-black transition-all flex items-center gap-3 shadow-xl active:scale-95">
            <i class="fa-solid fa-plus-circle text-lg text-blue-400"></i> NHẬP SÁCH MỚI
          </button>
        </div>
      </div>
              ...
              <tr v-for="book in bookList" :key="book._id" class="hover:bg-blue-50/30 transition-all group">
                <td class="px-8 py-5">
                  <div class="flex items-center gap-5">
                    <img :src="book.COVER" class="w-12 h-16 object-cover rounded-xl shadow-sm border border-slate-100" @error="(e) => e.target.src='https://placehold.co/400x600?text=No+Cover'" />
                    <div><p class="font-bold text-slate-800 group-hover:text-blue-600 transition-colors">{{ book.TENSACH }}</p><p class="text-[10px] font-bold text-slate-400 uppercase tracking-tighter">{{ book.TACGIA }} | <span class="text-blue-400">{{ book.THELOAI?.TenTheLoai }}</span></p></div>
                  </div>
                </td>
...
// 🔥 ĐÃ SỬA: Đổi API lấy Độc giả thành /api/user cho khớp với backend
const fetchReaders = async () => { 
  try { 
    const res = await axios.get('http://localhost:3000/api/user'); 
    readerList.value = res.data; 
  } catch (e) {
    console.error("Lỗi lấy danh sách Độc giả:", e);
  } 
};
...
const handleDeleteBook = async (id, name) => {
  const result = await Swal.fire({ title: 'Xóa sách?', text: `Xác nhận xóa cuốn "${name}"?`, icon: 'warning', showCancelButton: true, confirmButtonColor: '#ef4444', confirmButtonText: 'Xóa ngay' });
...
const filteredReaders = computed(() => {
  return readerList.value.filter(reader => {
    if (reader.role == 2 || reader.role == 3) return false;
    const isAlsoStaff = staffList.value.some(s => s.Username === reader.Username);
    return !isAlsoStaff; 
  });
});

const handleDeleteReader = async (id, name) => {
  const result = await Swal.fire({ title: 'Xóa độc giả?', text: `Bạn có chắc muốn xóa tài khoản của ${name}?`, icon: 'warning', showCancelButton: true, confirmButtonColor: '#ef4444', confirmButtonText: 'Xóa ngay' });
  if (result.isConfirmed) { 
    try { 
      // 🔥 ĐÃ SỬA: Đổi API xóa thành /api/user/
      await axios.delete(`http://localhost:3000/api/user/${id}`); 
      showNotification('Đã xóa độc giả!'); 
      fetchReaders(); 
    } catch (e) {
      Swal.fire('Lỗi', 'Không thể xóa! Có thể do Backend chưa hỗ trợ xóa.', 'error');
    } 
  }
};

const handleDeleteStaff = async (id, name) => {
  const result = await Swal.fire({ title: 'Xóa nhân viên?', text: `Bạn có chắc muốn xóa tài khoản của ${name}?`, icon: 'warning', showCancelButton: true, confirmButtonColor: '#ef4444', confirmButtonText: 'Xóa ngay' });
  if (result.isConfirmed) { try { await axios.delete(`http://localhost:3000/api/staff/${id}`); showNotification('Đã xóa nhân viên!'); fetchStaff(); } catch (e) {} }
};

onMounted(() => {
  fetchStats(); fetchBooks(); fetchStaff(); fetchReaders(); fetchOrders(); fetchCategories();
});
</script>